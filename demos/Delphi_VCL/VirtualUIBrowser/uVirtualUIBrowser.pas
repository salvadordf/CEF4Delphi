...
type
  TForm1 = class(TForm)
    ...
    procedure chrmosrAddressChange(Sender: TObject; const browser: ICefBrowser;
...
uses
  ...
  Math,
  {$ENDIF}
  uCEFMiscFunctions, uCEFApplication, uVirtualUIBrowserConstants;

// This demo is based on the SimpleOSRBrowser demo but it uses a different
// executable for the CEF subprocesses and it's configured to run inside VirtualUI

// We have to use the OSR mode to avoid refresh issues in VirtualUI.
// Chromium requires running several subprocesses and that can be problematic
// with VirtualUI. That's why we use a different executable for the subprocesses
// and call VirtualUI.AllowExecute before initializing CEF in uCEFLoader.pas.

// Read the code comments in the SimpleOSRBrowser demo for more information.
...
procedure TForm1.chrmosrAddressChange(Sender: TObject;
